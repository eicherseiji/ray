...
class AddEpisodeLengthsToTrainBatch(ConnectorV2):
    """Adds the EPS_LEN column with the correct episode lengths to train batch.

    - Operates on a list of Episode objects.
    - Gets all episode length(s) from all the given episodes
    and adds them to the batch under construction in the EPS_LEN column (as a list of
        ...
        **kwargs,
        ...
        for sa_episode in self.single_agent_episode_iterator(
            ...
            self.add_n_batch_items(
                batch,
                Columns.EPS_LENS,
                items_to_add=np.array(
                    [len(sa_episode)] * len(sa_episode), dtype=np.float32
                ),

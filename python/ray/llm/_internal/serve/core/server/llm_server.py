...
class LLMServer(LLMServerProtocol):
    ...
    async def _register_direct_ingress_app(self):
        ...
        This is called during start() after the engine is ready. The app is
        stored as self._asgi_app so that ASGIAppReplicaWrapper (which runs
        after __init__) picks it up instead of using the decorator's app.
        """
        try:
            ...
            # Set on self so ASGIAppReplicaWrapper.__init__ sees it.
            self._asgi_app = app

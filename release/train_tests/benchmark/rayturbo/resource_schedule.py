...
class MockResourceAvailabilityUpdater(ResourceAvailabilityUpdater):
    def __init__(self, starting_resource_units: int = 0, resource_key: str = "GPU"):
        ...
        logger.info(
            f"Initializing resource availability: '{resource_key}': {starting_resource_units}"
        )
    ...
    def _request(self, requester_ids):
        ...
        for requester_id in requester_ids:
            fut = self._coord.request_resources.remote(
                ...
                resources=[
                    {self._resource_key: 1.0}
                ],  # each represents a withheld "GPU"
                expire_after_s=10000,  # indefinite
    ...
    def _kill_random_train_worker(self):
        ...
        logger.info(f"Killing random train worker: {actor_to_kill}")
    ...
    def execute_schedule(self, schedule: List[ResourceAvailabilityEvent]):
        ...
        logger.info(f"Executing availability schedule: {schedule_str}")
        ...
        for event in schedule:
            ...
            logger.info(f"Executing scheduled event: {event}")
            ...
            if curr_available == event.resource_units:
                logger.info(
                    f"No change in availability: {curr_available} -> {event.resource_units}"
                )
                continue
            elif curr_available > event.resource_units:
                num_units_to_withold = curr_available - event.resource_units
                new_requesters = [
                    self._get_requester_id() for _ in range(num_units_to_withold)
                ]
                logger.info(
                    f"Reducing availability from {curr_available} to {event.resource_units}"
                )

                # If reducing resources, need to kill the actor processes / raylets.
                ...
                logger.info(
                    f"Increasing availability from {curr_available} to {event.resource_units}"
                )
...
def generate_schedule(
    ...
    for i in range(1, num_updates):
        # NOTE: Weights are arbitrarily chosen to bias generated schedules so that
        # schedules on average run with the maximum number of workers for ~50% of the time.
        weights = None
...
def cpu_example():
    """Example of how to schedule the availability of CPU resources."""
    ...
    schedule = generate_schedule(
        resource_availability_options=[1, 2, 4, 8],
        duration_s=60,
        interval_s=10,
        seed=42,
    )
    schedule_fut = updater_actor.execute_schedule.remote(schedule)

    train_id = "train"
    coord = get_or_create_autoscaling_coordinator()
    coord.request_resources.remote(
        requester_id=train_id,
        resources=[{"CPU": 1.0}] * 8,
    ...
    while schedule_fut:
        print(ray.get(coord.get_allocated_resources.remote(train_id)))

        _, schedule_fut = ray.wait(schedule_fut, timeout=0.5)

    ray.get(coord.cancel_request.remote(train_id))

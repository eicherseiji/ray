...
from rayturbo.resource_schedule import (
...
def main():
    ...
    # Set up the actor which sets the resource schedule in the background.
    ...
    # Update the metrics with the e2e throughput,
    # which includes recovery time across resource updates.
    total_rows_processed = metrics["train/rows_processed-total"]
    e2e_throughput = total_rows_processed / e2e_time
    metrics["e2e_throughput"] = e2e_throughput
    ...
    # TODO: If the update future is not done, that means training finished too quickly.
